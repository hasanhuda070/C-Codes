...
void checkSumInput(char *fileName, int checkSize){
	//File pointer checksum
	FILE *fileInput;
	//Variables
	int charValue = 0;
	int charCount = 0;
	int textLength = checkSize / 8 + 1;
	//Memory allocation
	...
	while(charValue != EOF){
		...
		if(textLength - i == 1){
			...
			checkSumRun(checkSize, charInput, checksum);
			j++;
			i = 0;
			...
			if(charValue == EOF){
				if(checkSize == 16 || checkSize == 32) //Check checksum size is 16 or 32 bits
				{
					//Check length
...
void checkSumRun(int checkSize, char *stringInput, unsigned int long *checksum){
	unsigned int long bitMask;
	bitMask = stringToBitMask(stringInput, checkSize);
...
unsigned int long stringToBitMask(char *stringInput, int checkSize){
	int i = 0;
	unsigned int long maskedBit;
	maskedBit = stringInput[i];
	checkSize -= 8;
	//While loop til checkSize is 0
	while(checkSize != 0){
		maskedBit = (maskedBit << 8) + stringInput[i + 1];
		checkSize -= 8;
		i++;
	}
	return maskedBit;
